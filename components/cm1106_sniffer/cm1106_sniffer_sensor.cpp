...
///////////////////////////////////////////////////////////////////////////////////////////////////
// We use ESPHome's UART device to receive the data from the CM1106 sensor.
// Setup is handled by the uart component (baud_rate: 9600 in YAML).
///////////////////////////////////////////////////////////////////////////////////////////////////
...
void CM1106SnifferSensor::loop()
{
  ...
  while (available >= 16) {
    this->read_array(response, 8);
    available = this->available();
  }

  // find the start of the message
  int matched = 0;
  while (matched < (int)sizeof(expected_header)) {
    if (this->available()) {
      this->read_array(response + matched, 1);
    } else {
  ...
  available = this->available();
  size_t remaining = sizeof(response) - matched;
  if (available < (int)remaining) {
  ...
  if (this->read_array(&response[matched], remaining))
    matched += remaining;
